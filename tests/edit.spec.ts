...
test("should navigate to dashboard -> campaigns -> edit", async ({ page }) => {
  ...
  const el = page.getByTestId("campaign-row-3");
  ...
  await page.waitForURL("http://localhost:3000/dashboard/campaigns");
  // await expect(page.getByTestId("campaign-name-3")).toContainText(
  //   "Chocolate Sauce",
  // );
  // await expect(page.getByTestId("campaign-budget-3")).toContainText(
  //   "$777.00",
  // );
